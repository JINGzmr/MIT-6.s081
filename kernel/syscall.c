...
// 它首先获取当前进程的指针 p，然后检查给定的地址 addr
// 是否有效，即是否在当前进程的地址空间范围内。 如果地址有效，它使用 copyin
// 函数从进程的页表中复制数据到 ip 指向的内存位置，并返回 0 表示成功，否则返回
// -1 表示失败。
...
// 它首先获取当前进程的指针 p，然后使用 copyinstr 函数从进程的页表中复制字符串到
// buf 指向的内存位置，最多复制 max 个字符。
...
// 每个系统调用都有一个唯一的编号（例如
// SYS_fork、SYS_exit），这些编号被用作索引来查找相应的系统调用函数。
...
// frame）中获取系统调用号（a7 寄存器中的值）。 如果系统调用号有效（大于 0
// 且小于数组 syscalls
// 的元素个数，并且对应的系统调用函数存在），则调用相应的系统调用函数，并将返回值存储在
// a0 寄存器中。 如果系统调用号无效，它会打印一条错误消息，然后将 -1 存储在 a0
// 寄存器中表示调用失败。
void syscall(void) {
    int num;
    struct proc* p = myproc();

...
1. Makefile（XV6目录下的文件）会读取一个C文件，例如**proc.c**

2. 之后调用**gcc编译器**，生成一个文件叫做**proc.s**，这是RISC-V 汇编语言文件
3. 后再走到**汇编解释器**，生成**proc.o**，这是汇编语言的二进制格式。

> Makefile会为所有内核的文件做相同的操作，并得到.o文件

4. 最后，系统加载器（Loader）会收集所有的.o文件，将它们**链接**在一起，并生成内核文件。
...
![image-20231207200948148](/home/zhangminrui/.config/Typora/typora-user-images/image-20231207200948148.png)
